...
public class Gerente extends Thread {
    ...
    private EstudioAnimacion plant;
    private int diasTotales;

    
    public Gerente(EstudioAnimacion plant, long dayDurationInMs) {
...
//        System.out.println("Gerente Trabajando");
    ...
    public void run() {
        while (true) {
            this.paySueldo();
            if(plant.entrega > 0){
                    System.out.println(plant.entrega);
...
//                    System.out.println("Gerente trabajando en la contabilidad a las " + (i / 2));
                ...
                try {
                    Thread.sleep(plant.dayDurationMs / 48); // Espera el intervalo de 30 minutos
            ...
            if(plant.isCartoon){
                ...
                dashboard.GlobalUI.getMainUI().getLGdashboard1().setDeadlineLabel(plant.entrega);                 
                // set dias totales UI
                dashboard.GlobalUI.getMainUI().getLGdashboard1().setDiasTotales(diasTotales);
            }else{
                // set dias restantes UI
                dashboard.GlobalUI.getMainUI().getRrDashboard1().setDeadlineLabel(plant.entrega);                 
                // set dias totales UI
                ...
                try {
                    Thread.sleep(plant.dayDurationMs/2);
